...
def upload_file(conn, localpath, remotepath):
    ...
    rf = conn.modules.builtins.open(remotepath, "wb")
    while True:
        buf = lf.read(16000)
        if not buf:
            break
...
def download_file(conn, remotepath, localpath):
    rf = conn.modules.builtins.open(remotepath, "rb")
...
def deliver(conn, localobj):
    ...
    return conn.modules.pickle.loads(pickle.dumps(localobj))
