...
static int query_formats(AVFilterContext *ctx){
    ...
    avfilter_set_common_formats(ctx,avfilter_make_format_list(1,PIX_FMT_RGB24));
    return 0;
}
...
static void end_frame(AVFilterLink *link){
    ...
    //独自構造体に代入
    vhext_frame frame;
    frame.data = pic->data[0];
    frame.linesize = pic->linesize[0];
    frame.w = pic->w;
    frame.h = pic->h;
    frame.pts = ((double)pic->pts) / AV_TIME_BASE;
	//ライブラリを呼び出す。
