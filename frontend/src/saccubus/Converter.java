...
public class Converter extends Thread {
	...
	private boolean checkOK() {
		...
		//	if (UserSession.isEmpty() && Setting.isBrowserSafari()){
		//		BrowserKind = BrowserCookieKind.Safari;
		//		UserSession = BrowserInfo.getUserSession(BrowserKind);
		//	}
	...
	private NicoClient getNicoClient() {
		if (isSaveVideo() || isSaveComment() || isSaveOwnerComment()) {
			...
			if (BrowserKind != BrowserCookieKind.NONE){
				...
				client = new NicoClient(BrowserKind, UserSession, proxy, proxy_port);
			} else {
				client = new NicoClient(getMailAddress(), getPassword(), proxy, proxy_port);
			}
			if (!client.isLoggedIn()) {
				sendtext("ログインに失敗 " + BrowserInfo.getBrowserName());
			} else {
