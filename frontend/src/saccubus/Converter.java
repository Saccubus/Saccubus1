...
public class Converter extends Thread {
	...
	private NicoClient getNicoClient() {
		if (isSaveVideo() || isSaveComment() || isSaveOwnerComment()) {
			...
			sendtext("ログイン中");
			NicoClient client = new NicoClient(getMailAddress(), getPassword(),
					proxy, proxy_port);
...
//			.replace('.', '．')		// .(dot) is let there
	...
	private int converting_video(String vhook_path, String vhookowner_path) {
		...
		if (fwsFile == null) {
			...
			ffmpeg.addFile(fwsFile);
		}
		ffmpeg.addCmd(" ");
		ffmpeg.addCmd(OutOption);
		if (!Setting.isVhookDisabled()) {
			if(!addVhookSetting(ffmpeg, vhook_path, vhookowner_path)){
				return -1;
			}
...
//		ffmpeg.addCmd(" -an -vcodec copy -f image2 -vbsf mjpeg2jpeg ");
	...
	private boolean addVhookSetting(FFmpeg ffmpeg, String vhook_path,
			String vhookowner_path) {
		try {
			ffmpeg.addCmd(" -vfilters \"");
			if (!getFFmpegVfOption().isEmpty()){
				ffmpeg.addCmd(getFFmpegVfOption());
			...
			ffmpeg.addCmd("vhext=");
			ffmpeg.addCmd(Setting.getVhookPath().replace("\\", "/"));
			ffmpeg.addCmd("|");
			if(CommentMiddleFile!=null){
			...
			ffmpeg.addCmd("|");
			ffmpeg.addCmd("--shadow:");
			ffmpeg.addCmd("" + Setting.getShadowIndex());
			ffmpeg.addCmd("|");
