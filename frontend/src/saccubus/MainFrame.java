...
//	JButton CheckFFmpegInputButton = new JButton();
	...
	private void jbInit() throws Exception {
		GridBagConstraints grid7_x2_y1_74 = new GridBagConstraints(2, 1, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 0, 0, 5), 0, 0);
//		grid7_x2_y1_74.gridwidth = 1;
...
//		GridBagConstraints grid11_x0_y3_80 = new GridBagConstraints();
		...
		StatusPanel.add(elapsedTimeBar,BorderLayout.NORTH);
		...
		grid7_x1_y0_84.insets = new Insets(5, 5, 5, 5);
		...
		grid7_x2_y0_83.insets = new Insets(0, 0, 5, 5);
	...
	public void FFVersionButton_actionPerformed(ActionEvent e){
		try{
			...
			TextFFmpegOutput.setText("(´∀｀)＜ぬるぽ　ガッ");
			ex.printStackTrace();
	...
	public void CheckDownloadVideoButton_actionPerformed(ActionEvent e){
		...
		try {
			...
			if (converter == null){
				ConvertingSetting set = getSetting();
				converter = new Converter(
						VideoID_TextField.getText(),
						WayBackField.getText(), set, statusBar,
						new ConvertStopFlag(new JButton(), "", "", ""));
				if (set.isVideoFixFileName()) {
					File folder = set.getVideoFixFileNameFolder();
					folder.mkdir();
			...
			TextFFmpegOutput.setText(null);
			String inOption = "";
			if (converter.detectOption()){
				inOption = converter.getInOption();
			}
			ArrayList<String> list = execFFmpeg("-y " + inOption
