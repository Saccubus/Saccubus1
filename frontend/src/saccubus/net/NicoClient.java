		...
		try {
			...
			for (int i = 0; i < array.length; i++) {
				...
				if (ThreadID == null && key.equalsIgnoreCase("thread_id")) {
					...
					NeedsKey = true;
				} else if (key.equalsIgnoreCase("is_premium")) {
					Premium = value;
...
*/
	...
	public File getComment(final File file, final JLabel status, String back_comment, String time) {
		if (time != null && !time.equals("") && WayBackKey.equals("0")){
			if (!getWayBackKey(time)) { // WayBackKey
				// System.out.println("It may be wrong Date.");
				return null;
			}
	...
	private boolean getWayBackKey(String time) {
		...
		if (ret == null ||
			...
			System.out
				.println("ng.\nCannot find WAYBACKKEY_STR from response.");
			return false;
		}
