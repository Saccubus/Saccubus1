...
public class NicoClient {
	...
	public NicoClient(final String user, final String pass,
			final String proxy, final int proxy_port) {
		User = user;
		Pass = pass;
		nicomap = new NicoMap();
...
//	private final static String WAYBACKKEY_STR = "waybackkey=";
	...
	private static final String LOGIN_USERNAME_TAG = "<p id=\"login_username\">";
	private static final String MYPAGE_TITLE_TAG = "<title>マイページ";
	private static final String IDNUM_TAG = "<dt class=\"idNum\">ID:</dt>";
	...
	private boolean loginCheck() {
		...
		try {
			HttpURLConnection con = urlConnectGET(url);
			if (con == null){
				System.out.println("ng.\nCan't read mypage at loginCheck:" + url);
				return false;
			}
			String new_cookie = detectCookie(con);
			if (new_cookie == null || new_cookie.isEmpty()) {
				System.out.println("ng.\nloginCheck: cannot get cookie.");
				/*
				con.disconnect();
				return false;
				*/
			...
			while ((ret = br.readLine()) != null) {
				Stopwatch.show();
				debug("■readLine(" + encoding + "):" + ret + "\n");
				if (ret.indexOf(MYPAGE_TITLE_TAG) >= 0) {
					loginOK = true;
					break;
				}
				if (ret.indexOf(IDNUM_TAG) >= 0){
					loginOK = true;
					break;
				}
			}
