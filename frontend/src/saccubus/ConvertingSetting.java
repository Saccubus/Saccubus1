...
public class ConvertingSetting {
			...
			int shadow_index)
	{
		...
		// CAUTION!! Folowings line must be fixed !!
	...
	private static final String PROP_ADD_TIMESTAMP = "AddTimeStamp";
		// Add Timestamp to comment filename, when using multiple comment files as logs.
	...
	public static ConvertingSetting loadSetting(String user, String password) {
		...
		return new ConvertingSetting(
			...
			prop.getProperty(PROP_OWNERCOMMENT_FILE, ".\\comment" + saccubus.Converter.OWNER_EXT),
			...
			prop.getProperty(PROP_CMDLINE_OUT,"-threads 2 -s 512x384 -acodec libmp3lame -ab 128k -ar 44100 -ac 2 -vcodec libxvid -qscale 3 -async 1"),
