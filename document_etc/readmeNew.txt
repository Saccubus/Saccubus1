...
　　/(((docomo|iPhone) (white )?)?18[46])|(18[46])/
　　		デフォルトコマンドのコメントをNG
...
●CA（コメントアート）対応　その５
　・CAフォント追加　MingLiU（繁字体）、NGULIM(漢字XP)、Arial Unicode(XP)、
　　estrangelo edessa(シリア)、（計１０種）
　・「CA暫定的対応」はフォント指定を増やす機能とサイズ自動調整のみ残し
　　その他のCA対応機能は「開発版を有効」の方に移動しました。
...
　・saccubus.iniファイルがあるときには初期化時に読み込みます。
　①には従来通り入っていません。
　②はffmpeg55686用のsaccubus.iniを同梱。削除すると①と同じになります。

●設定ファイルの読み込み・保存・別名保存
　・ファイルメニューから設定ファイル(*.xml)の開く・上書き保存・
　　名前を付けて保存が出来るようになりました。
...
■収録物（配布形態は①②の二種類があります）
　　①ffmpeg55686無し版　N55686を再DLしない人向け、主にアップデート用
　　②ffmpeg55686同梱版　主に初めて使用する人向け
□フォルダ構成
　saccubud1.30.zip
　├ffmpeg55686無し版/ffmpeg55686同梱版  表示
　├最初に必ず読んで.txt
　└さきゅばすファイル一式　　本体jar xml bat
　　├bin        実行ファイルフォルダ exe dll
　　├option     オプションフォルダ(r21400用)
　　├optionDev  オプションフォルダ(実験用)
...
・Saccubus.jar　　　　　　　本体　1.30
・Bin.jar　　　　　　　　　 ランタイムライブラリ
・bin フォルダ配下
　　ffmpeg55686.exe ライセンスGPLV3 ②のみ
　　nicovideoE.dll(2012.02.08版)統合版拡張Vhookライブラリ
　　SDL.dll(2011.11.02ビルド)、README-SDL.txt
　　SDL_ttf.dll(2011.11.02ビルド)、COPYNG
　　　グラフィックライブラリ、ライセンスはLGPL
・option フォルダ配下　　　 変換オプションファイル
　　　（r21400他従来用 2012.02.08版）
・optionDev フォルダ配下 　 実験用ファイル
