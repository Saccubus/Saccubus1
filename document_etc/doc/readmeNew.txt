...
□拡張機能（変更点）説明
●FFmpegのDLL探索パス設定修正(1.67.7.11)

●flashページアクセス用のクッキー追加。(1.67.7.10)

●Unicode表示を有効にする。(1.67.7.02)
　ファイル名やログ出力表示でUnicodeのままにする。(従来はshift-jis)
　オンにするとログ出力表示が文字化けすることがある。
　ログファイルはUTF-8エンコードになる。

●変換中画像小画面
　変換中画像表示のサイズを小さくします。(1.67.7.01)

●バグ修正 1.67.6.27,1.67.6.28(1.67.6.29)
●サムネ表示(1.67.6.28)
　実行中のサムネをアイコン表示します。
●dmc且つエコノミー且つdmc/smile両方保存時のバグ修正(#70)(1.67.6.27)
●右クリックから削除時リストソートが乱れるのを修正,リストエコ表示バグ修正(1.67.6.26)
●自動終了の時間制限設定(1.67.6.25-26)
　変換実行後、制限時間内に変換がないと自動的に終了する。その値を設定する。
　制限時間を設定するとメニューバーに残り時間が表示される。
　終了はさきゅばす終了であってWindowsのシャットダウンではない。

●ActivityPane状況表示修正(1.67.6.24)
　右クリックメニューからのアイテム削除とコピーIDを追加
●mylistGetterでも時刻表示(1.67.6.24)
●login失敗時ステータスのloginメッセージを消す(1.67.6.24)
●生放送TSの運営コメント秒数指定のデグレード修正(1.67.6.23b)
●session Api "api_urls"→"urls"対応(1.67.6.23)
●xmlがない時Jsonのxml変換を行う。(1.67.6.21)
●コミュニティ動画でdmc動画のsmileサイズ取得ミス修正(1.67.6.20)
