...
　nakaコメント表示が左端ではなく途中から表示されるのを変更可能に(issue #38)
　公式ではnakaはvposの1秒前にレイヤー判定されるから１秒内のue.shitaによって非表示になるが
　衝突の基準をvappearで判定することにした。合わせて追い越しェック追加
　コメントスピードチェックボックスでオンオフ
　(スピードは変更しても数値指定せず空白にして変更しなくてもこのモードになる)
