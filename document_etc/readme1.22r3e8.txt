...
　・1.22r3e8 (2011/8/18)　新コメント表示対応（いんきゅばす1.7.0相当）
...
　既にさきゅばすを使用中の方
　・Saccubus.jarを差し替えます。
　・1.22r3e8のbinフォルダ内のファイルを使用中のbinフォルダに
　　コピーして下さい。
...
　　代わりにnicovideo.dllを使うとエラーにならないということです。
　　そのため、XMLがない時のデフォルト設定を変更しています。
　　DLでは問題は未発生ですがエラー時にはnicovideo.dllに変更して下さい。
...
　・1.22r3e8を1.22r3を展開したフォルダにコピーします。
...
　・実行方法
　　AUTO2PROC.BAT　　　　　2P自動実行本体…ダブルクリックで起動
　　AUTO2PROCDEBUG.BAT　　 2P自動実行時のログ記録用…ダブルクリックで起動
...
・サイト
　さきゅばす公式　http://saccubus.sourceforge.jp/
