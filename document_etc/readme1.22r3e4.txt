...
　・1.22r3e4 (2011/8/3)　ワイド画面コメントサイズ対応(仮)
...
　既にさきゅばすを使用中の方
　・Saccubus.jarを差し替えます。
　・1.22r3e4のbinフォルダ内のファイルを
　　使用中のbinフォルダにコピーして下さい。
...
　　をダウンロードし展開します。
　　binフォルダの FFmpeg, vhookライブラリ(従来)nicovideo.dllなど全てと
　　optionフォルダ　が必要です。
　　*** 同梱しているbinフォルダのファイルだけでは動作しません ***
　・1.22r3e4を1.22r3を展開したフォルダにコピーします。
...
●拡張Vookワイド（nicovideo16x9.dll）について
　・従来のnicovideo.dllより、16:9動画での字の大きさはニコニコ公式に
　　近いですが完全な再現は未達成です。
　・拡張Vhookの指定方法は次の通りです。
　　２つ選択時→DL動画を検査してアスペクト比を判定しVhookを選択
　　１つ選択時→指定されたVhookを使用して変換、無検査
　　０選択→エラー
　　■コメント付き動画(変換後)のアスペクト比を変更したり判定したり■
　　■はしないので注意してください。　　　　　　　　　　　　　　　■
...
・readme1.22r3e4.txt、readme.txt(1.22r)、readme+.txt(1.22r3)
・saccubus1.22r3e4_src.zip
　　1.22r3e4 javaソースファイル、eclipseプロジェクトファイル
・autodebug.bat　　auto.batのログ記録用バッチファイル
・bin\nicovideo16x9.dll　　　雪羽氏作成の16:9動画用拡張Vhookライブラリ
　http://yukihane.dip.jp/dist/imcubus/20101017dll/nicovideo.zip
　注↑サーバ停止中のようです。
　http://cyclotron.mine.nu/up/small/src/cyclotron_s3232.rar
　スレの住人さんによるうｐ
・bin\libSDL_gfx-13.dll　　　グラフィックライブラリ,ライセンスはLGPL
　nicovideo16x9.dllに必要
　

変更履歴
1.22r3e4 (2011/8/3)
　ワイド画面(16;9)コメントサイズ対応(仮)
